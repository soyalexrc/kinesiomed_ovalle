        ...
        <div className="flex justify-center">
            <div className="px-10 md:px-20 py-24 max-w-[1400px] w-full">
                ...
                <div className="grid grid-cols-2 mt-12 relative">
                    <div className="w-[370px] h-[540px] absolute"
                         style={{top: '50%', left: '50%', transform: 'translate(-50%, -50%)'}}>
                        <Image src="/why-us.png" fill alt="why us"/>
                    </div>
                    {data.map((item, index) => (
                        <div key={item.id}
                             className={`col-span-2 px-8 md:col-span-1 border-b-[1px] flex items-center gap-4 h-[180px] ${index % 2 !== 0 ? `flex-row-reverse bg-brand ${index === 1 ? 'rounded-tr-2xl' : index === 5 ? 'rounded-br-2xl' : ''}` : `bg-brandSecondary/10 ${index === 0 ? 'rounded-tl-2xl' : index === 4 ? 'rounded-bl-2xl' : ''}`}`}>
                            <div
                                className={`rounded-2xl w-[52px] h-[52px] ${index % 2 === 0 ? 'bg-brandSecondary/10 ' : 'bg-white'}`}/>
